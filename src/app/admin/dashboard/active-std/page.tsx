...
export default function ActiveStdPage() {
  const router = useRouter();
  const [items, setItems] = useState<ActiveStd[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch("/api/admin/active-std");
        const data = await res.json();
        setItems(data.items || []);
      } catch (error) {
        console.error("Failed to fetch active STDs:", error);
      } finally {
        setLoading(false);
      }
    }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-200">
                ...
                <th className="text-left px-4 py-3 font-medium text-stone-500">Weergaven</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-stone-400">
                    Geen actieve Save the Dates gevonden
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr
                    key={item.id}
                    onClick={() => router.push(`/admin/dashboard/active-std/${item.id}`)}
